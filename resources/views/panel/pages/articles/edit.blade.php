        ...
        <div class="col-lg-6">
            <div class="iq-card">
                ...
                <div class="iq-card-body">
                    <form action="/panel/articles/{{ $article->id }}" method="POST" enctype="multipart/form-data" id="form-submit" onsubmit="formOnSubmit()">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="title">Judul<span class="text-danger">*</span></label>
                            <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" placeholder="Judul" value="{{ old('title') ? old('title') : $article->title }}">
                            @error('title')
                            ...
                            <select class="form-control @error('article_category_id') is-invalid @enderror" id="article_category_id" name="article_category_id">
                                ...
                                @foreach ($articleCategories as $articleCategory)
                                    <option value="{{ $articleCategory->id }}" @if (old('article_category_id')) {{ old('article_category_id') == $articleCategory->id ? 'selected' : '' }}
                                            @else
                                            {{ $articleCategory->id == $articleCategory->id ? 'selected' : '' }} @endif>{{ $articleCategory->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <img src="{{ $article->thumbnail_url }}" alt="{{ $article->title }}" class="img-thumbnail d-block" width="150px" id="thumbnail-preview">
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" name="content" id="content" value="{{ old('content') ? old('content') : $article->content }}">
                            @error('content')
                                <div class="text-danger small">
                                    {{ $message }}
                                </div>
